...
/><div className="flex flex-1 flex-col p-5 sm:p-6"><div className="mb-4 flex items-center gap-2 font-mono text-[10px] uppercase tracking-[.16em] text-acm"><CalendarDays size={13} /> {event.date}</div><h3 className="font-display text-xl font-semibold leading-tight text-white">{event.title}</h3><p className="mt-3 flex-1 text-sm leading-6 text-slate-400">{event.description}</p><div className="mt-5 flex flex-wrap gap-2">{event.tags.map(tag => <span key={tag} className="rounded-full border border-white/10 px-2.5 py-1 text-[10px] text-slate-400">{tag}</span>)}</div></div></article> }
